...
export class VariablesBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    try {
      ...
      const assignments = [
        {
          variableId: 'variable block',
          variableName: 'apiLabel',
          type: 'string',
          value: 'Get value API',
        },
        {
      ...
      // --- GHI VÀO customExportStore TRONG CONTEXT ---
      // Ghi data vào customExportStore trong context
      if (context.customExportStore) {
        // Ví dụ 1: set từng key
        context.customExportStore.set('apiLabel', 'Get value API')
        context.customExportStore.set('count', 123)

        // Ví dụ 2: set cả object variables
        context.customExportStore.set('variables', output)
      }
