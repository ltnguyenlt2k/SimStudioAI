...
export class Executor {
  ...
  async execute(
    ...
    try {
      ...
      while (hasMoreLayers && iteration < maxIterations && !this.isCancelled) {
        ...
        if (this.isDebugging) {
          ...
          if (nextLayer.length === 0) {
            ...
            // NEW: sync ra store
            useCustomExportStore.setState({
              data: context.customExportStore?.data ?? {},
            })
            ...
            return {
              ...
              metadata: {
                ...
                // NEW: đưa data từ store ra ngoài
                customExportData: context.customExportStore?.data ?? {},
              },
      ...
      // NEW: sync ra store trước khi trả kết quả
      useCustomExportStore.setState({
        data: context.customExportStore?.data ?? {},
      })
  ...
  private createExecutionContext(
    ...
    const context: ExecutionContext = {
      ...
      // ====== NEW: customExportStore ======
      customExportStore: {
        data: customExportData,
        set: (key: string, value: any) => {
          customExportData[key] = value
        },
        get: <T = any>(key: string): T | undefined => {
          return customExportData[key] as T | undefined
        },
      },
    }
  ...
  private checkDependencies(
    ...
    return incomingConnections.every((conn) => {
      ...
      // For condition blocks: once the condition block has executed,
      // ALL outgoing branches are considered "ready".
      // We vẫn lưu selectedCondition trong context để UI dùng,
      // nhưng không dùng nó để chặn các nhánh khác nữa.
      if (conn.sourceHandle?.startsWith('condition-')) {
        const sourceBlock = this.actualWorkflow.blocks.find((b) => b.id === conn.source)
        if (sourceBlock?.metadata?.id === BlockType.CONDITION) {
          // Chỉ cần block điều kiện đã chạy là dependency thoả
          return sourceExecuted
        }
      }
  ...
  private async executeBlock(
    ...
    try {
      ...
      // Nếu là Condition block: luôn kích hoạt tất cả nhánh (if / elseif / else)
      if (block.metadata?.id === BlockType.CONDITION) {
        try {
          const outgoingConnections =
            context.workflow?.connections.filter((conn) => conn.source === block.id) || []

          for (const conn of outgoingConnections) {
            // Thêm toàn bộ target của Condition vào activeExecutionPath
            context.activeExecutionPath.add(conn.target)
          }
        } catch (e) {
          logger.warn(
            `Failed to activate all condition branches for block ${block.id}:`,
            e
          )
        }
      }
      ...
      if (
        ...
        // NEW: nếu là Condition block, kích hoạt tất cả nhánh (if / elseif / else)
        if (block.metadata?.id === BlockType.CONDITION) {
          const allPaths = (output as any).allPaths
          if (Array.isArray(allPaths)) {
            for (const path of allPaths) {
              if (path && path.blockId) {
                context.activeExecutionPath.add(path.blockId)
              }
            }
          }
        }
